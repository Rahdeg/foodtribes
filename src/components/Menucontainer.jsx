      ...
      <div className="w-full flex flex-col  items-center justify-between">
        ...
        <div className=" w-full flex items-center justify-start lg:justify-center gap-8 py-6 overflow-x-scroll scrollbar-none">
         {categories && categories.map((item,idx)=>(
          <motion.div 
          whileTap={{scale:0.75}}
          key={idx} className={`group ${filter===item.urlParamName ?' bg-cartNumBg':' bg-card'}  w-24 min-w-[94px] h-24 cursor-pointer hover:bg-red-600 rounded-lg drop-shadow-xl flex flex-col gap-3 items-center justify-center `}
          onClick={()=>setFilter(item.urlParamName)}  >
          <div className={`w-10 h-10 rounded-full shadow-lg ${filter===item.urlParamName ?' bg-card':' bg-cartNumBg'} group-hover:bg-card flex items-center justify-center`}>
          <IoFastFood className={`${filter===item.urlParamName ?' text-textColor':' text-white'}  group-hover:textColor text-lg`}/> 
          </div>
