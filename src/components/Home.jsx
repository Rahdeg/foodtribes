...
const Home = () => {
  return (
    ...
    <p className=' font-bold text-[2.5rem] lg:text-[4.5rem] tracking-wide text-headingColor'>The Fastest Delivery in <span className=' text-orange-600 text-[3rem] lg:text-[5rem]'>Your City</span></p>
    ...
    <button type='button' className=' bg-gradient-to-br from-orange-400 to bg-orange-500 w-full px-4 py-2 rounded-lg hover:shadow-lg transition-all ease-in-out duration-100 md:w-auto'>Order Now</button>
    </div>
    <div className=' py-2 flex-1 flex items-center relative'>
        <img src={home} alt='hm' className=' h-370 lg:h-650 w-full lg:w-auto ml-auto'/>
    <div className=' w-full h-full absolute  top-0 left-0 flex items-center justify-center py-4 gap-4 flex-wrap lg:px-32'>
    {heropdata && heropdata.map((data,idx)=>(    
    <div className=' lg:w-190   p-4 bg-cardOverlay backdrop-blur-md  rounded-3xl flex flex-col items-center justify-center drop-shadow-lg' key={idx}>
    <img src={data.imgsrc}  alt='' className='w-20 lg:w-40  -mt-10  lg:-mt-20 items-center justify-center'  />
    <p className=' text-base lg:text-xl font-semibold text-textColor mt-2 lg:mt-4'>{data.name} </p>
    <p className=' text-[10px] lg:text-sm  text-lighttextGray font-semibold  my-1 lg:my-3'>{data.descp} </p>
    <p className=' text-xm font-semibold text-headingColor '><span className=' text-xs text-red-500'>$ </span>{data.price} </p>
    </div>
    ))}
    </div>
