...
export const saveUser= async(data)=>{
    await setDoc(doc(firestore,'userData',`${Date.now()}`),data,{merge:true})
}
...
export const savePayment= async(data)=>{
    await setDoc(doc(firestore,'paymentDetails',`${Date.now()}`),data,{merge:true})
}
...
export const saveBike= async(data)=>{
    await setDoc(doc(firestore,'bikeDetails',`${Date.now()}`),data,{merge:true})
}
